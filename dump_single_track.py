...
async def main():
    ...
    # Dump the HTML of the entire track list area
    html = await page._page.evaluate('''() => {
        // Expand the first track just in case
        const headers = [...document.querySelectorAll('div')].filter(d => d.innerText && d.innerText.match(/^[0-9]+$/));
        if(headers.length > 0) headers[0].click();
        
        return document.body.innerHTML;
    }''')
    
    with open("track_details_dump.html", "w") as f:
        f.write(html)
        
    print("Dumped track_details_dump.html")
    await browser.close()
