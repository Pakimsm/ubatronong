...
async def main():
    ...
    classes = await page._page.evaluate('''() => {
        let els = [...document.querySelectorAll('*')];
        let unggah = els.find(el => el.innerText === 'Unggah file lagu lengkap');
        if(!unggah) return "Not found";
        
        let trackContainer = unggah.closest('li') || unggah.parentElement.parentElement.parentElement;
        return trackContainer.className;
    }''')
    
    print("Track container class:", classes)
