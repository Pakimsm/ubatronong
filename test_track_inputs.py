...
async def main():
    ...
    fields = await page._page.evaluate('''() => {
        let panel = document.querySelector('.semi-collapse-panel');
        if(!panel) return "No panel found!";
        
        let els = [...panel.querySelectorAll('*')];
        let texts = [];
        els.forEach(el => {
            if(el.children.length === 0 && el.innerText && el.innerText.trim() !== '') {
                texts.push(el.innerText.trim());
            } else if (el.tagName === 'INPUT') {
                texts.push(`[INPUT: ${el.type} | val: ${el.value} | ph: ${el.placeholder}]`);
    ...
    with open('track_texts.json', 'w') as f:
        json.dump(fields, f, indent=2)
        
    print("Saved track_texts.json")
    await browser.close()
