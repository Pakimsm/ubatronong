...
async def main():
    ...
    # Scroll down slightly and take screenshots
    for i in range(3):
        await page._page.evaluate(f"window.scrollBy(0, 500)")
        await asyncio.sleep(1)
        await page._page.screenshot(path=f"track_form_part_{i}.png", full_page=True)
        print(f"Saved track_form_part_{i}.png")
